...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, backbone='cspdarknet', pretrained=False, input_shape=[960, 960], attention=False):
        super(YoloBody, self).__init__()
        depth_dict          = {'s' : 0.33, 'm' : 0.67, 'l' : 1.00, 'x' : 1.33,}
        width_dict          = {'s' : 0.50, 'm' : 0.75, 'l' : 1.00, 'x' : 1.25,}
        dep_mul, wid_mul    = depth_dict[phi], width_dict[phi]
        
        self.attention = attention # 控制输出注意力机制权重的开关
        ...
        if backbone == "cspdarknet":
            ...
            #   20,20,1024
            #---------------------------------------------------#
            self.backbone   = CSPDarknet(base_channels, base_depth, phi, pretrained, attention=attention)
        else:
            #---------------------------------------------------#   
    ...
    def forward(self, x):
        ...
        if not self.attention:
            ...
            feat1, feat2, feat3, attention_map = self.backbone(x)
