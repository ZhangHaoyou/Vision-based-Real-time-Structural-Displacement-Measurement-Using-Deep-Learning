...
├── predict_video.py           # Track one user-selected target through an arbitrary recorded video (Section 5.8)
...
└── detections/                    # Script outputs: cropped/annotated detection images, per-video output folders
    └── shaking_table_test.mp4      # Demo video (tracked as a .gitignore exception -- see README.md)
```
...
### 5.8 `predict_video.py` — track one user-selected target in any video (general-purpose)

A general-purpose script for tracking a single target of your choosing through an arbitrary recorded video, with no code changes required. Workflow:

1. Move the mouse to position an **exact 960×960 crop box** (the model's native input size, read from `yolo.input_shape`) on the first frame, and left-click to confirm — every frame is then cropped to that exact window before detection, instead of the whole frame being squeezed down to fit the model, which hurts detection of small targets.
2. **Drag a rectangle** inside that crop around the target you want to track (the ROI).
3. Optionally zero out everything outside the ROI.
4. Optionally give the target's real-world width in cm, to get displacement in mm as well as pixels.
5. Optionally track with **Deep SORT** (`--use-deepsort`, or answer `y` at the prompt) so the target keeps its identity through brief occlusion or if it temporarily leaves/re-enters the ROI; without it, each frame independently picks whichever detected box is fully inside the ROI with the highest confidence.
...
Outputs go to `./detections/frames/<video_name>/` (annotated frames), `./detections/videos/<video_name>_detected.mp4` (full annotated video), and `./detections/motion/<video_name>/` (`motion.csv` with per-frame pixel/mm displacement, `config.txt` recording the crop box/ROI/settings used, and a displacement plot).

Note: a frame where the target isn't detected gets **no row at all** in `motion.csv` (not interpolated, not NaN-filled) — so the `frame` column can have gaps. The annotated video still includes every frame (labelled "target not detected"), but those frames aren't part of the displacement data or plot.
