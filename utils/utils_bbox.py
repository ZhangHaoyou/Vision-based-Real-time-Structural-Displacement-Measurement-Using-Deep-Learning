...
class DecodeBox():
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4, wnms=True):
        #----------------------------------------------------------#
            ...
            for c in unique_labels:
                ...
                if wnms:
                    ...
                    while detections_class.size(0):
                        ...
                        large_overlap = bbox_iou(detections_class[0, :4].unsqueeze(0), detections_class[:, :4]) > nms_thres
                        ...
                        weights = detections_class[invalid, 4:5]
            
                        # Merge overlapping bboxes by order of confidence
                        # 这里得到最后的bbx它是跟他满足IOU大于threshold，并且相同label的一些bbx，根据confidence重新加权得到
                        # 并不是原始bbx的保留。
                        detections_class[0, :4] = (weights * detections_class[invalid, :4]).sum(0) / weights.sum()
                        keep_boxes += [detections_class[0].unsqueeze(0)]
                    ...
                    max_detections = torch.cat(keep_boxes).data
